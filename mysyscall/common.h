...
#define PRINT_PTRACER_PREFIX "[ PTRACER ] "
...
#define print_ptracer(format...) print_info_internal(PRINT_PTRACER, format)
#define print_ptracer_no_prompt(format...) print_info_internal(PRINT_PTRACER | PRINT_NO_PROMPT, format)
...
#define BEGIN_INTERCEPT if (!begin_intercept(CUR_SYSCALL)) return ret
#define BEGIN_INTERCEPT_NR(nr_syscall) if (!begin_intercept(nr_syscall)) return ret

// call it before return to restore errno
static inline void end_intercept() {
    errno = saved_errno;
}
#define END_INTERCEPT do { end_intercept(); return ret; } while (0)
