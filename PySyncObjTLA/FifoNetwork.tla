...
(***************************************************************************
  API InitFifoNetwork(nodes):
    ...
    - _netcmd: <<"init">>
 ***************************************************************************)
InitFifoNetwork(nodes) ==
    /\ _msgs = [ sender \in nodes |-> [ recver \in nodes \ {sender} |-> <<>> ]]
    /\ _netman = [ n_sent |-> 0, n_recv |-> 0, n_wire |-> 0, conn |-> <<nodes>>,
                   n_part |-> 0, n_cure |-> 0]
    /\ _netcmd = <<"init">>
TypeOkFifoNetwork(nodes, msgType) ==
    /\ _msgs \in [ nodes -> [ nodes -> [ Nat -> msgType ]]]
    /\ _netman \in [ n_sent: Nat, n_recv: Nat, n_wire: Nat ]
    /\ _netcmd \in [ Nat -> Any ]
...
NetDelMsg(m) ==
    ...
    IN <<NetDecWire @@ NetIncRecv, res[2], <<"msg_del", m.src, m.dst>>>>
...
\* Last modified Sun Jul 17 10:38:08 CST 2022 by fedora
\* Last modified Thu May 05 21:38:16 CST 2022 by tangruize
